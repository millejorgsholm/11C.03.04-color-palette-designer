...
//Creating empty array and declaring "color" variable
const hslArr = [];
const Color = {
  h: 0,
  s: 0,
...
function calculateHarmony(hsl) {
  ...
  for (let i = 0; i < radioButtons.length; i++) {
    if (
      ...
      radioButtons[i].value === "analogous"
    ) {
      calcAnalogous(hsl);
    }
    if (
      radioButtons[i].checked === true &&
      radioButtons[i].value === "monochromatic"
    ) {
    ...
    if (
      radioButtons[i].checked === true &&
      radioButtons[i].value === "complementary"
    ) {
      calcComplementary(hsl);
    }
    if (
      radioButtons[i].checked === true &&
      radioButtons[i].value === "compound"
    ) {
      calcCompound(hsl);
    }
    if (
      radioButtons[i].checked === true &&
      radioButtons[i].value === "shades"
    ) {
      calcShades(hsl);
    }
  }
...
function showHSL(hsl, index) {
  ...
  document.querySelector(
    `#colorinfo${index + 1} .hsl`
  ).textContent = `H: ${h} S: ${s}% L: ${l}%`;
}

//Changing the colorbox
function changeBoxColor(css, index) {
  document.querySelector(
    `#colorinfo${index + 1} .colorbox`
